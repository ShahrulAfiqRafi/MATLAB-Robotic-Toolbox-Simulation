...
L(3).qlim = [0 100]; %min and max prismatic length

scara=SerialLink(L) %checking DH table
scara.name = 'SCARA Robot'; %Naming the robot
scara.plot([0 0 100 0], 'workspace',[-50 150 -100 100 -200 100]); %Setting up the workspace

%plotting based on degrees of Link 1 and 2
t=0:0.5:1; %time frame
...
%animation from one point to another
...
%Foward Kinematics calculation that can be found in Workspace
...
%trajectory tracking of the movement of the robot
for i=1:1:length(t)
    T1=tr1(i);
    trc1=transl(T1);
    xx1(i)=trc1(1);
...
hold on
%setting up the line tracking color and width
